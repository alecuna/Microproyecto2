...
// Función para insertar los documentos de videojuegos en Firestore
async function insertarVideojuegos() {
  try {
    const videojuegosCollection = collection(db, "games");
    for (const juego of videojuegos) {
      const juegoDocRef = doc(videojuegosCollection, juego.ID);
      await setDoc(juegoDocRef, juego); // Establecer los datos del documento antes de insertarlo
      console.log(`Videojuego ${juego.titulo} insertado correctamente.`);
    }
    console.log("Todos los videojuegos han sido insertados.");
  } catch (error) {
    console.error("Error al insertar videojuegos:", error);
  }
}

// Función para insertar los documentos de clubes en Firestore
async function insertarClubes() {
  try {
    const clubesCollection = collection(db, "clubes");
    for (const club of clubes) {
      const clubDocRef = doc(clubesCollection, club.ID);
      await setDoc(clubDocRef, club);
      console.log(`Club ${club.nombre} insertado correctamente.`);
    }
    console.log("Todos los clubes han sido insertados.");
  } catch (error) {
    console.error("Error al insertar clubes:", error);
  }
}

// Ejecutar las funciones para insertar los datos
insertarVideojuegos();
insertarClubes();
